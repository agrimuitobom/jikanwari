...
import type { CreateInput, Subject } from '../../types'
import { downloadCsvTemplate, parseCsv } from '../../utils/csvSubjects'
import { ErrorAlert } from '../common/ErrorAlert'
import { GRADE_LABELS } from '../../utils/constants'
import type { Grade } from '../../types'

interface SubjectCsvImportProps {
  onImport: (subjects: CreateInput<Subject>[]) => Promise<void>
  onClose: () => void
}

export function SubjectCsvImport({ onImport, onClose }: SubjectCsvImportProps) {
  const fileRef = useRef<HTMLInputElement>(null)
  const [parsed, setParsed] = useState<CreateInput<Subject>[] | null>(null)
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      const { subjects, errors } = parseCsv(text)
      setParseErrors(errors)
      setParsed(subjects.length > 0 ? subjects : null)
    }
  ...
  return (
    <div className="card p-6 sm:p-8 space-y-6">
      <div className="flex items-center justify-between border-b border-gray-200 pb-4">
        <h2 className="text-lg font-semibold text-gray-900">CSV一括登録</h2>
      ...
      <section className="space-y-2">
        ...
        <p className="text-sm text-gray-500">
          CSVファイルのフォーマットに沿って科目情報を入力してください。サンプルデータ入りです。
        </p>
        <button type="button" onClick={downloadCsvTemplate} className="btn-secondary">
      ...
      {parsed && !done && (
        <section className="space-y-3">
          ...
          <div className="max-h-64 overflow-auto rounded-lg border border-gray-200">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 sticky top-0">
                <tr>
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">科目名</th>
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">学年</th>
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">教科</th>
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">単位</th>
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">コマ/週</th>
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">連続</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {parsed.map((s, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-gray-600">{GRADE_LABELS[s.grade as Grade]}</td>
          ...
          <p className="text-xs text-gray-400">{parsed.length}件の科目を登録します</p>
